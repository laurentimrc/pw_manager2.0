...
export function CardList() {
  ...
  const refresh = useCallback(async () => {
    try {
      const result = await api.listCards(debouncedSearch, tag)
      setData(result)
      setError(null)
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Errore nel caricamento delle carte.')
    }
    ...
    <div className="flex flex-col gap-4">
      ...
      {data && (
        <p className="text-sm text-muted-foreground">
          {data.filtered_total} di {data.total} carte
        </p>
      )}

      {data && data.filtered_total === 0 && <Alert variant="default">Nessuna carta trovata.</Alert>}
